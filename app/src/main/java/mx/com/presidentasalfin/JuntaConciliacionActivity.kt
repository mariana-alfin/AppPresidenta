...
class JuntaConciliacionActivity : CameraBaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title =
            HtmlCompat.fromHtml("<font color='#FFFFFF' face='montserrat_medium_italic'>${
                FuncionesGlobales.convertPesos(saldoConciliar,
                    2)
            }</font>", HtmlCompat.FROM_HTML_MODE_LEGACY);
        supportActionBar?.subtitle =
            HtmlCompat.fromHtml("<font color='#FFFFFF' face='montserrat_extra_bold_italic'>Saldo por Conciliar</font>",
        ...
        for (pgo in listPagos) {
            try {
                ...
                if (txtPago.text.isNotEmpty()) {                      //SOLO SI NO ESTA VACIO SE AGREGA ALA SUMA
                    ...
                    findViewById<TextView>(R.id.txtSuma).text =
                        FuncionesGlobales.convertPesos(sumaPagos, 2)
