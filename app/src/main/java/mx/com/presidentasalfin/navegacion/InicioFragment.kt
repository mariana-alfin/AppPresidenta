...
import mx.com.presidentasalfin.databinding.Inicio2FragmentBinding
import mx.com.presidentasalfin.generales.FuncionesGlobales
import mx.com.presidentasalfin.generales.FuncionesGlobales.Companion.setMaxLength
...
class InicioFragment : Fragment() {
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        (activity as AppCompatActivity).supportActionBar?.title =
            HtmlCompat.fromHtml("<font face='montserrat_extra_bold_italic'>Inicio</font>",
                    ...
                    Response.Listener { response ->
                        try {
                            ...
                            if (jsonData.getInt("code") == 200)//si la peticion fue correcta se continua con el login
                            {
                                ...
                                txtBonificacionAcumulada.text =
                                    "  ${FuncionesGlobales.convertPesos(1235.0, 0)}  "
                                txtBonificacion.text =
                                    "  ${FuncionesGlobales.convertPesos(289.0, 0)}  "
                                txtPago.text =
                                    FuncionesGlobales.convertPesos(jsonResults.getDouble("pay"), 0)
                                /*MD SE GUARDA EN SESSION EL MONTO PAGO DEL CREDITO PARA LA VISTA DE PAGOS*/
                                FuncionesGlobales.guardarVariableSesion(requireActivity(),
                                    "Float",
                                    "MONTO_SEMANAL",
                                    jsonResults.getString("pay"))
                                FuncionesGlobales.guardarVariableSesion(requireActivity(),
                                    "String",
                                ...
                                txtMtoPag.text =
                                    FuncionesGlobales.convertPesos(jsonResults.getDouble("payments"),
                                        0)
                                txtPagos.text = jsonResults.getString("due_pay")
                                txtSaldoVencido.text =
                                    FuncionesGlobales.convertPesos(jsonResults.getDouble("min_pay"),
                                        0)
    ...
    private fun solicitarSoporte() {
        ...
        builder.setTitle(HtmlCompat.fromHtml("<font color='#1F2C49' size='20' face='montserrat_medium_italic'>DESCRIBE TU PROBLEMA</font>",
            HtmlCompat.FROM_HTML_MODE_LEGACY))
