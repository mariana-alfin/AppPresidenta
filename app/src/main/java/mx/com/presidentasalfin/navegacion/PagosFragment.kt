...
import mx.com.presidentasalfin.JuntaActivity
import mx.com.presidentasalfin.JuntaConciliacionActivity
import mx.com.presidentasalfin.R
import mx.com.presidentasalfin.databinding.PagosFragmentBinding
...
class PagosFragment : Fragment() {
    ...
    private fun verJuntas(fechaPago: String, juntas: JSONArray) {
        ...
        try {
            for (i in 0 until juntas.length()) {
                ...
                listaJuntas.add("Junta #${junta.getString("number")} Monto Capturado ${
                    FuncionesGlobales.convertPesos(junta.getDouble("amount"),
                        2)
                }")
                listaNJuntas.put(i,
                    juntaPgo(junta.getInt("number"),
                        junta.getDouble("amount"),
                        junta.getInt("pay_id")))
            }
        } catch (e: java.lang.Exception) {
        ...
        if (juntas.length() != 0) {
            ...
            builder.setPositiveButton("Aceptar") { dialog, which ->
                ...
                if (task_number != 0 && idPago != 0) {
                    ...
                    val detalleJunta =
                        Intent(activity, mx.com.presidentasalfin.DetalleJuntaActivity::class.java)
                    //MD ENVIAMOS DATOS
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        (activity as AppCompatActivity).supportActionBar?.title =
            HtmlCompat.fromHtml("<font face='montserrat_extra_bold_italic'>Calendario de Pagos</font>",
