...
class HistorialActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title =
            HtmlCompat.fromHtml("<font color='#FFFFFF' face='montserrat_extra_bold_italic'>Mi Historial</font>",
    ...
    private fun mostrarFormato(esMostrar: Boolean) {
        ...
        findViewById<TextView>(R.id.txt).visibility = valor
    }
    ...
    private fun llenarTablaHistorial(jsonResults: JSONArray) {
        ...
        listRad.clear()
        val prefs = PreferenceManager.getDefaultSharedPreferences(this)
        val creditoSesion = prefs.getInt("CREDITO_ID", 0)
        for (i in 0 until numCreditos) {
            ...
            grupo.text =
                cte.getString("group_name") + "  \n " + HtmlCompat.fromHtml("<font color='#FDCB6E'>  $idCredito </font>",
                    HtmlCompat.FROM_HTML_MODE_LEGACY)

            grupo.setTextColor(colorAzul)
            grupo.textSize = fontTr
            grupo.typeface = tipoLetraTr
            //se obtiene el idPrestamo con estatus 1 que es el activo
            if (idCredito == creditoSesion) {
                radIdCliente.isChecked = true
            }
            if (cte.getInt("status") == 1) {
                grupo.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.ic_estrella, 0);
            }
        ...
        if (creditoSesion != 0) {
            findViewById<TextView>(R.id.txtHistorico).text = "Credito seleccionado: $creditoSesion"
        }
        mostrarFormato(true)
    }
